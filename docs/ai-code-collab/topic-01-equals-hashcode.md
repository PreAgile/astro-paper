...
```java
// ⚠️ 이 코드에는 세 가지 문제가 있습니다 (이런 패턴은 권장하지 않습니다):
...
> **추가 문제: 스레드 안전성 — 단순히 ConcurrentHashSet으로 바꾸면 될까?**
>
...
>
> **⚠️ add-first 패턴의 핵심 — 실패 시 반드시 정리하기**:
>
...
> **Medium에서 보시는 분**: 아래 다이어그램이 렌더링되지 않으면 [mermaid.live](https://mermaid.live)에서 코드를 붙여넣어 확인하세요.

```mermaid
graph LR
    subgraph "정상적인 경우"
        A1["PaymentId 생성"] --> A2["hashCode 계산"]
        A2 --> A3["버킷 5 저장"]
        A4["동일 값으로 조회"] --> A5["같은 hashCode"]
        A5 --> A6["버킷 5 탐색"]
        A6 --> A7["equals 비교"]
        A7 --> A8["찾음"]
    end
...
```mermaid
graph LR
    subgraph "hashCode 미재정의 시"
        B1["PaymentId A 저장"] --> B2["Object.hashCode=12345"]
        B2 --> B3["버킷 5 저장"]
        B4["PaymentId B 조회"] --> B5["Object.hashCode=67890"]
        B5 --> B6["버킷 10 탐색"]
        B6 --> B7["비어있음"]
    end
```
...
```mermaid
graph TB
    subgraph "KAFKA-1194: 모든 파티션이 한 버킷에"
        H["HashMap"]
        H --> B0["Bucket 0: 비어있음"]
        H --> B1["Bucket 1: 비어있음"]
